      ...
      if (!_grid.getEditorLock().isActive() || _grid.getOptions().autoEdit) {
        ...
        if ((e.which === keyCodes.C || e.which === keyCodes.INSERT) && (e.ctrlKey || e.metaKey) && !e.shiftKey) {    // CTRL+C or CTRL+INS
          if (_onCopyInit) {
        	...
        	// callback함수 리턴값으로 다음 로직 진행여부 판단
            if(!_onCopyInit.call(null, e)) {
            	return false;
            }
          }
                ...
                for (var i=range.fromRow; i< range.toRow+1 ; i++){
                    ...
                    if (clipTextRows == "" && _options.includeHeaderWhenCopying) {
                        ...
                        for (var j = range.fromCell; j < range.toCell + 1 ; j++) {
                        	...
                        	// 헤더컬럼이 있고, 기존과 다를경우 진행
                            if (columns[j].name.length > 0 && columns[j].name !== currHeader) {
                            	currHeader = columns[j].name;
                            	clipTextHeaders.push(getHeaderValueForColumn(columns[j]));
                            }
                        }
                        
                        // 헤더컬럼이 있을경우 진행
                        if(clipTextHeaders['length'] > 0) {
                        	clipTextRows.push("g_r_i_d_"+clipTextHeaders.join("\t"));
                        }
                    }
